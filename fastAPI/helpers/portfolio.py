...
class Portfolio():
    ...
    def __init__(self, tickers:list[str], weights: list[int]):
    ...
    def tickers_exist(self):
        ...
        if len(error_list) == 0:
            self.exist = True
            self.excluded = error_list
        else:
            self.exist = False
            self.excluded = error_list
        
    # Import data
    def getData(self, start_date:datetime, end_date:datetime) -> pd.DataFrame:
        ...
        self.df = df 
        self.excluded = excluded_tickers
    
    
    # return and volatility
    def return_performance(self, weights, returns_annual):
        weights /= np.sum(weights)
        return_perf = np.dot(weights, returns_annual)
        return return_perf
    
    def volatility_performance(self, weights, cov_annual):
        weights /= np.sum(weights)
    ...
    def optimize(self):
        ...
        returns_daily = self.df.pct_change()
        ...
        actual_volatility = self.volatility_performance(self.weights, cov_annual)
        actual_return = self.return_performance(self.weights, returns_annual)    
        ...
        for counter,symbol in enumerate(self.tickers):
            actual_portfolio[symbol+' weight'] = self.weights[counter]
        ...
        for single_portfolio in range(num_portfolios):
            ...
            returns = self.return_performance(self.weights, returns_annual)  
            volatility = self.volatility_performance(self.weights, cov_annual)
        ...
        def plot_weights ():
            fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5,1,figsize=(len(self.tickers)*1.5, 12))
            fig.tight_layout(pad=2.5)
            plt.axis('equal')

            sns.barplot(actual_portfolio_df.iloc[:,-len(self.tickers):], color='orange', ax=ax1)
            sns.barplot(min_variance_port.iloc[:,-len(self.tickers):], color='green', ax=ax2)
            sns.barplot(sharpe_portfolio.iloc[:,-len(self.tickers):], color='red',ax=ax3)
            sns.barplot(min_volatility_for_return.iloc[:,-len(self.tickers):], color='blue', ax=ax4)
            sns.barplot(max_return_for_volatility.iloc[:,-len(self.tickers):], color='blue', ax=ax5)
        ...
        return { 
                ...
                'Max_Return_for_actual_volatility' : {
                                                    ...
                                                    'weights': max_return_for_volatility.iloc[:,-len(self.tickers):].to_dict('records')
                                                    },
                'Min Volability for actual Return' : {
                                                    'actual_return': min_volatility_for_return['Returns'].values[0],
                                                    'weights': min_volatility_for_return.iloc[:,-len(self.tickers):].to_dict('records')
                                                    }
                }
